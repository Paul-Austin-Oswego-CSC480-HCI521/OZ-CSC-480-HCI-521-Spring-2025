...
test.describe('Test Cases', () => {
    ...
    test('TC_041: Verify guests cannot access the profile page (LOG_041)', async ({ page }) => {
        await page.goto('http://localhost:9080/');
        await page.getByRole('button', { name: 'Continue as Guest' }).click();
        const profileIcon = page.getByRole('listitem', { name: /Account Page/i });
        await expect(profileIcon).toHaveCount(0); // Assert it doesn’t exist at all
    });
